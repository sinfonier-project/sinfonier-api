...
class RunTopology(BaseHandler):
	...
	def post(self):
		if self.arguments:
			...
			# Parse each param
			data = self.arguments
			if 'name' not in data.keys() or 'xml' not in data.keys() or 'reload' not in data.keys():
			...
			try:
				...
				try:
					...
					u.checkAndcreate(options.storm_topology_path+str(t_name)+options.storm_topology_config_path,"storm","storm")
					try:
						# Create folder /data/storm/topologies/{name}/jar if not exists
						u.checkAndcreate(options.storm_topology_data_path+str(t_name)+options.storm_topology_jar_path,"storm","storm")
					except Exception, e:
						api_logger.error("Error on Create folder "+options.storm_topology_data_path+str(t_name)+options.storm_topology_jar_path+" if not exists. "+str(e))
				except Exception, e:
					api_logger.error("Error on Create folder "+options.storm_topology_path+str(t_name)+options.storm_topology_config_path+" if not exists. "+str(e))
			except Exception, e:
			...
			if t_reload:
				...
				jar_path = options.storm_topology_data_path+str(t_name)+options.storm_topology_jar_path+storm_global_jar_bin
			#
			# Create xml file 
			# 		/var/storm/{topologyname}/config/
			try:
				with open(options.storm_topology_path+str(t_name)+options.storm_topology_config_path+str(t_name)+".xml", "w") as builder_file:
			...
			cmd_launch = options.storm_binary+" jar "+jar_path+" com.sinfonier.DynamicTopology "+options.storm_topology_path+str(t_name)+options.storm_topology_config_path+str(t_name)+".xml "+str(t_name)
			...
			if err:
				...
				self.finish('{"result":"error","description":"Error launching topology", "detail":"'+str(error)+'"}')
			else:
				...
				for line in output.splitlines():
					if "ERROR" in line:
						...
						break
				if errorDebug:
					self.finish('{"result":"error","description":"Error launching topology", "detail":"'+str(dedugStack)+'"}')
				else:	
			...
			if 'name' not in data.keys():
				...
				if topo_info:
					...
					if response["status"] == "KILLED":
						...
						self.write('{"result":"error","description":"Topology "'+t_name+'" could not be stopped","debug":""}')
			...
			if topo_info:
				...
				if response["status"] == "KILLED":
					...
					if err:
						...
						moncon.updateStoppedState(t_name)
						moncon.mongoStopCon()
						self.finish('{"result":"error","description":"Error launching topology","detail":"'+str(err)+'"}')
					else:
